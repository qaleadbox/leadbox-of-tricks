...
function renderEditFromPreview(a) {
	...
	link.href = `${location.origin}/leads/edit/${id}`;
	link.className = 'lbot-edit-icon lbot-additional-icons';
	link.title = 'Edit Lead';
	link.style.marginLeft = '8px';
	link.setAttribute('data-action', 'edit-lead');
	...
	if (printer) cell.insertBefore(link, printer);
	else a.parentNode.insertBefore(link, a.nextSibling);
}
